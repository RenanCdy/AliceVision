...
void exportSimilarityVolumeCross(const CudaHostMemoryHeap<TSimRefine, 3>& in_volumeSim_hmh,
                                 const CudaHostMemoryHeap<sycl::float2, 2>& in_depthSimMapSgmUpscale_hmh,
                                 const mvsUtils::MultiViewParams& mp, int camIndex, const RefineParams& refineParams,
                                 const std::string& filepath, const ROI& roi)
{
    sfmData::SfMData pointCloud;
    ...
    for(int vy = 0; vy < volDim[1]; ++vy)
    {
        ...
        for(int vx = xIdxStart; vx < xIdxStop; ++vx)
        {
            ...
            for(int vz = 0; vz < volDim[2]; ++vz)
            {
                ...
                const double depth =
                    depthPixSizeMap.x() +
                    (relativeDepthIndexOffset * depthPixSizeMap.y()); // original depth + z based pixSize offset
...
void exportSimilarityVolumeTopographicCut(const CudaHostMemoryHeap<TSimRefine, 3>& in_volumeSim_hmh,
                                          const CudaHostMemoryHeap<sycl::float2, 2>& in_depthSimMapSgmUpscale_hmh,
                                          const mvsUtils::MultiViewParams& mp, int camIndex,
                                          const RefineParams& refineParams, const std::string& filepath, const ROI& roi)
{
    sfmData::SfMData pointCloud;
    ...
    for(size_t vx = 0; vx < volDim.x(); ++vx)
    {
        ...
        for(size_t vz = 0; vz < volDim.z(); ++vz)
        {
            ...
            const int relativeDepthIndexOffset = vz - refineParams.halfNbDepths;
            const double depth = depthPixSizeMap.x() + (relativeDepthIndexOffset *
                                                        depthPixSizeMap.y()); // original depth + z based pixSize offset
...
inline rgb float4_to_rgb(const sycl::float4& v)
{
    return {float_to_uchar(v.x()), float_to_uchar(v.y()), float_to_uchar(v.z())};
}

void exportColorVolume(const CudaHostMemoryHeap<sycl::float4, 3>& in_volumeSim_hmh, const std::vector<float>& in_depths,
                       int startDepth, int nbDepths, const mvsUtils::MultiViewParams& mp, int camIndex, int scale,
                       int step, const std::string& filepath, const ROI& roi)
{
    sfmData::SfMData pointCloud;
    int xyStep = 10;
    ...
    for (int vy = 0; vy < volDim[1]; vy += xyStep)
    {
        for (int vx = 0; vx < volDim[0]; vx += xyStep)
        {
            ...
            for(int vz = 0; vz < nbDepths; ++vz)
            {
                ...
                sycl::float4 colorValue =
                    *get3DBufferAt_h<sycl::float4>(in_volumeSim_hmh.getBuffer(), spitch, pitch, vx, vy, vz);
