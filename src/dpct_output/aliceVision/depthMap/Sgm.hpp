...
public:
     ...
     */
    Sgm(const mvsUtils::MultiViewParams& mp, const mvsUtils::TileParams& tileParams, const SgmParams& sgmParams,
        bool computeDepthSimMap, bool computeNormalMap, dpct::queue_ptr stream);
    ...
    inline const CudaDeviceMemoryPitched<sycl::float2, 2>& getDeviceDepthThicknessMap() const {
        return _depthThicknessMap_dmp;
    }

    // final depth/similarity map getter (optional: could be empty)
    inline const CudaDeviceMemoryPitched<sycl::float2, 2>& getDeviceDepthSimMap() const { return _depthSimMap_dmp; }

    // final normal map getter (optional: could be empty)
    inline const CudaDeviceMemoryPitched<sycl::float3, 2>& getDeviceNormalMap() const { return _normalMap_dmp; }
...
private:
    ...
    CudaDeviceMemoryPitched<sycl::float2, 2> _depthThicknessMap_dmp; //< rc result depth thickness map
    CudaDeviceMemoryPitched<sycl::float2, 2> _depthSimMap_dmp;       //< rc result depth/sim map
    CudaDeviceMemoryPitched<sycl::float3, 2> _normalMap_dmp;         //< rc normal map
