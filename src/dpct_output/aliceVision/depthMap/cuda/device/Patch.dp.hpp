...
/*
DPCT1110:2: The total declared local variable size in device function rotPointAroundVect exceeds 128 bytes and may cause
...
*/
inline void rotPointAroundVect(sycl::float3& out, sycl::float3& X, sycl::float3& vect, int angle)
{
    double ux, uy, uz, vx, vy, vz, wx, wy, wz, sa, ca, x, y, z, u, v, w;

    double sizeX = sycl::sqrt(dot(X, X));
    x = X.x() / sizeX;
    y = X.y() / sizeX;
    z = X.z() / sizeX;
    u = vect.x();
...
inline void rotatePatch(Patch& ptch, int rx, int ry)
{
    sycl::float3 n, y, x;
...
inline void movePatch(Patch& ptch, int pt)
{
    // float3 v = ptch.p-rC;
    // normalize(v);
    sycl::float3 v = ptch.n;

    float d = ptch.d * (float)pt;
    sycl::float3 p = dpct_operator_overloading::operator+(ptch.p, dpct_operator_overloading::operator*(v, d));
    ptch.p = p;
}

inline void computeRotCS(sycl::float3& xax, sycl::float3& yax, sycl::float3& n)
{
                                        ...
                                        const DeviceCameraParams& tcDeviceCamParams)
{
    ...
    sycl::float3 v1 = dpct_operator_overloading::operator-(rcDeviceCamParams.C, ptch.p);
    // Vector from the target camera to the 3d point
    sycl::float3 v2 = dpct_operator_overloading::operator-(tcDeviceCamParams.C, ptch.p);
    normalize(v1);
    normalize(v2);
    ...
    ptch.n =
        dpct_operator_overloading::operator/((dpct_operator_overloading::operator+(v1, v2)), 2.0f); // IMPORTANT !!!
...
inline float computePixSize(const DeviceCameraParams& deviceCamParams, const sycl::float3& p)
{
    const sycl::float2 rp = project3DPoint(deviceCamParams.P, p);
    const sycl::float2 rp1 = dpct_operator_overloading::operator+(rp, sycl::float2(1.0f, 0.0f));
...
inline void getPixelFor3DPoint(sycl::float2& out, const DeviceCameraParams& deviceCamParams, const sycl::float3& X)
{
    ...
    if(p.z() <= 0.0f)
        ...
        out = sycl::float2(p.x() / p.z(), p.y() / p.z());
}

inline sycl::float3 get3DPointForPixelAndFrontoParellePlaneRC(const DeviceCameraParams& deviceCamParams,
...
inline sycl::float3 get3DPointForPixelAndDepthFromRC(const DeviceCameraParams& deviceCamParams, const sycl::float2& pix,
                                                     float depth)
{
    sycl::float3 rpv = M3x3mulV2(deviceCamParams.iP, pix);
    normalize(rpv);
    return dpct_operator_overloading::operator+(deviceCamParams.C, dpct_operator_overloading::operator*(rpv, depth));
}

inline sycl::float3 triangulateMatchRef(const DeviceCameraParams& rcDeviceCamParams,
                                        const DeviceCameraParams& tcDeviceCamParams, const sycl::float2& refpix,
                                        const sycl::float2& tarpix)
{
    sycl::float3 refvect = M3x3mulV2(rcDeviceCamParams.iP, refpix);
    normalize(refvect);
    const sycl::float3 refpoint = dpct_operator_overloading::operator+(refvect, rcDeviceCamParams.C);

    sycl::float3 tarvect = M3x3mulV2(tcDeviceCamParams.iP, tarpix);
    normalize(tarvect);
    const sycl::float3 tarpoint = dpct_operator_overloading::operator+(tarvect, tcDeviceCamParams.C);
    ...
    return dpct_operator_overloading::operator+(rcDeviceCamParams.C, dpct_operator_overloading::operator*(refvect, k));
}
...
inline void computeRcTcMipmapLevels(float& out_rcMipmapLevel, float& out_tcMipmapLevel, const float mipmapLevel,
                                    const DeviceCameraParams& rcDeviceCamParams,
                                    const DeviceCameraParams& tcDeviceCamParams, const sycl::float2& rp0,
                                    const sycl::float2& tp0, const sycl::float3& p0)
{
    // get p0 depth from the R camera
    const float rcDepth = size(dpct_operator_overloading::operator-(rcDeviceCamParams.C, p0));

    // get p0 depth from the T camera
    const float tcDepth = size(dpct_operator_overloading::operator-(tcDeviceCamParams.C, p0));

    // get R p0 corresponding pixel + 1x
    const sycl::float2 rp1 = dpct_operator_overloading::operator+(rp0, sycl::float2(1.f, 0.f));

    // get T p0 corresponding pixel + 1x
    const sycl::float2 tp1 = dpct_operator_overloading::operator+(tp0, sycl::float2(1.f, 0.f));

    // get rp1 3d point
    sycl::float3 rpv = M3x3mulV2(rcDeviceCamParams.iP, rp1);
    normalize(rpv);
    const sycl::float3 prp1 =
        dpct_operator_overloading::operator+(rcDeviceCamParams.C, dpct_operator_overloading::operator*(rpv, rcDepth));

    // get tp1 3d point
    sycl::float3 tpv = M3x3mulV2(tcDeviceCamParams.iP, tp1);
    normalize(tpv);
    const sycl::float3 ptp1 =
        dpct_operator_overloading::operator+(tcDeviceCamParams.C, dpct_operator_overloading::operator*(tpv, tcDepth));
...
inline int angleBetwUnitV1andUnitV2(sycl::float3& V1, sycl::float3& V2)
{
    return (int)sycl::fabs(sycl::acos(V1.x() * V2.x() + V1.y() * V2.y() + V1.z() * V2.z()) / (CUDART_PI_F / 180.0f));
}
...
/*
DPCT1110:3: The total declared local variable size in device function computeHomography exceeds 128 bytes and may cause
high register pressure. Consult with your hardware vendor to find the total register size available and adjust the code,
or use smaller sub-group size to avoid high register pressure.
*/
inline void computeHomography(float* out_H, const DeviceCameraParams& rcDeviceCamParams,
                              const DeviceCameraParams& tcDeviceCamParams, const sycl::float3& in_p,
                              const sycl::float3& in_n)
{
    // hartley zisserman second edition p.327 (13.2)
    const sycl::float3 _tl = dpct_operator_overloading::operator-(sycl::float3(0.0, 0.0, 0.0),
                                                                  M3x3mulV3(rcDeviceCamParams.R, rcDeviceCamParams.C));
    const sycl::float3 _tr = dpct_operator_overloading::operator-(sycl::float3(0.0, 0.0, 0.0),
                                                                  M3x3mulV3(tcDeviceCamParams.R, tcDeviceCamParams.C));

    const sycl::float3 p =
        M3x3mulV3(rcDeviceCamParams.R, (dpct_operator_overloading::operator-(in_p, rcDeviceCamParams.C)));
    sycl::float3 n = M3x3mulV3(rcDeviceCamParams.R, in_n);
    normalize(n);
    ...
    const sycl::float3 tr = dpct_operator_overloading::operator-(_tr, M3x3mulV3(tmpRr, _tl));

    float tmp[9];
    float tmp1[9];
    outerMultiply(tmp, tr, dpct_operator_overloading::operator/(n, d));
...
inline float compNCCby3DptsYK(
    ...
    /*
    FIXED-DPCT1050:20: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
    */
    const dpct::image_accessor_ext<sycl::float4, 2> rcMipmapImage_tex,
    /*
    FIXED-DPCT1050:21: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
    */
    const dpct::image_accessor_ext<sycl::float4, 2> tcMipmapImage_tex,
    const unsigned int rcLevelWidth, const unsigned int rcLevelHeight, const unsigned int tcLevelWidth,
    const unsigned int tcLevelHeight, const float mipmapLevel, const int wsh, const float invGammaC,
    const float invGammaP, const bool useConsistentScale, const Patch& patch)
{
    // get R and T image 2d coordinates from patch center 3d point
    const float2 rp = project3DPoint(rcDeviceCamParams.P, patch.p);
    const float2 tp = project3DPoint(tcDeviceCamParams.P, patch.p);
...
inline float compNCCby3DptsYK_customPatchPattern(
    const DeviceCameraParams& rcDeviceCamParams, const DeviceCameraParams& tcDeviceCamParams,
    /*
    FIXED-DPCT1050:22: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
    */
    const dpct::image_accessor_ext<sycl::float4, 2> rcMipmapImage_tex,
    /*
    FIXED-DPCT1050:23: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
    */
    const dpct::image_accessor_ext<sycl::float4, 2> tcMipmapImage_tex,
    const unsigned int rcLevelWidth, const unsigned int rcLevelHeight, const unsigned int tcLevelWidth,
    const unsigned int tcLevelHeight, const float mipmapLevel, const float invGammaC, const float invGammaP,
