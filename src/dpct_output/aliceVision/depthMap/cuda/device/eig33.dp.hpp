...
inline double cuda_hypot2(double x, double y)
{
    return sycl::sqrt(x * x + y * y);
}

// Symmetric Householder reductio3 to tridiago3al form.

static void cuda_tred2(double V0[], double V1[], double V2[], double d[], double e[])
{
    int i, j, k;
...
/*
DPCT1110:0: The total declared local variable size in device function cuda_tql2 exceeds 128 bytes and may cause high
...
*/
static void cuda_tql2(double V0[], double V1[], double V2[], double d[], double e[])
{
    int i, l, m, iter, k, j;
...
static void cuda_eigen_decomposition(double A[3][3], double V0[], double V1[], double V2[], double d[])
{
    double e[3];
...
struct cuda_stat3d
{
    ...
    void update(const sycl::float3& p, const float& w)
    {
    ...
    /*
    DPCT1110:1: The total declared local variable size in device function getEigenVectorsDesc exceeds 128 bytes and may
    cause high register pressure. Consult with your hardware vendor to find the total register size available and adjust
    the code, or use smaller sub-group size to avoid high register pressure.
    */
    void getEigenVectorsDesc(sycl::float3& cg, sycl::float3& v1, sycl::float3& v2, sycl::float3& v3, float& d1,
                             float& d2, float& d3)
    {
        ...
        v1 = sycl::float3((float)V[0][2], (float)V[1][2], (float)V[2][2]);
        normalize(v1);
        v2 = sycl::float3((float)V[0][1], (float)V[1][1], (float)V[2][1]);
        normalize(v2);
        v3 = sycl::float3((float)V[0][0], (float)V[1][0], (float)V[2][0]);
        normalize(v3);
    ...
    bool computePlaneByPCA(sycl::float3& p, sycl::float3& n)
    {
        if(count < 3.0)
        {
            return false;
