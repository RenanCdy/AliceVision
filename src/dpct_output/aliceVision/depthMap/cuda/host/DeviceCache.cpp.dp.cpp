...
sycl::float3 M3x3mulV3(const float* M3x3, const sycl::float3& V)
{
...
void fillHostCameraParameters(DeviceCameraParams& cameraParameters_h, int camId, int downscale, const mvsUtils::MultiViewParams& mp)
{
    ...
    cameraParameters_h.XVect = M3x3mulV3(cameraParameters_h.iR, sycl::float3(1.f, 0.f, 0.f));
    normalize(cameraParameters_h.XVect);

    cameraParameters_h.YVect = M3x3mulV3(cameraParameters_h.iR, sycl::float3(0.f, 1.f, 0.f));
    normalize(cameraParameters_h.YVect);

    cameraParameters_h.ZVect = M3x3mulV3(cameraParameters_h.iR, sycl::float3(0.f, 0.f, 1.f));
...
void fillDeviceCameraParameters(const DeviceCameraParams& cameraParameters_h, int deviceCameraParamsId)
 try {
    const dpct::memcpy_direction kind = dpct::host_to_device;
    const dpct::err0 err = DPCT_CHECK_ERROR(dpct::get_default_queue()
                                                .memcpy((char*)(constantCameraParametersArray_d.get_ptr()) +
                                                            deviceCameraParamsId * sizeof(DeviceCameraParams),
                                                        &cameraParameters_h, sizeof(DeviceCameraParams))
                                                .wait());
...
void DeviceCache::addCameraParams(int camId, int downscale, const mvsUtils::MultiViewParams& mp)
 try {
    ...
    const dpct::err0 err = cudaMallocHost(&cameraParameters_h, sizeof(DeviceCameraParams));
    ...
    DPCT_CHECK_ERROR(sycl::free(cameraParameters_h, dpct::get_default_queue()));
}
catch(sycl::exception const& exc) {
