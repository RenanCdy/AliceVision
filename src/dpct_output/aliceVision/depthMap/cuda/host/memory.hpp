...
using CudaColorBaseType = sycl::half;
#else
...
public:
    ...
    void copyFrom(const CudaDeviceMemoryPitched<Type, Dim>& src, dpct::queue_ptr stream = &dpct::get_default_queue());
...
public:
    ...
    // see below with copy() functions
    void copyFrom(const CudaDeviceMemoryPitched<Type, Dim>& src, dpct::queue_ptr stream = &dpct::get_default_queue());
    void copyFrom(const CudaHostMemoryHeap<Type, Dim>& src, dpct::queue_ptr stream = &dpct::get_default_queue());
...
public:
    ...
    CudaDeviceMemory(const Type* inbuf, const size_t size, dpct::queue_ptr stream)
    {
        allocate( size );
    ...
    void copyFrom(const Type* inbuf, const size_t num, dpct::queue_ptr stream)
    {
        cudaMemcpyKind kind = cudaMemcpyHostToDevice;
...
void CudaDeviceMemoryPitched<Type, Dim>::copyFrom(const CudaDeviceMemoryPitched<Type, Dim>& src, dpct::queue_ptr stream)
{
...
template <class Type, unsigned Dim>
void CudaDeviceMemoryPitched<Type, Dim>::copyFrom(const CudaHostMemoryHeap<Type, Dim>& src, dpct::queue_ptr stream)
{
...
template <class Type, unsigned Dim>
void CudaHostMemoryHeap<Type, Dim>::copyFrom(const CudaDeviceMemoryPitched<Type, Dim>& src, dpct::queue_ptr stream)
{
...
template <class Type>
void copy2D(Type* dst, size_t sx, size_t sy, Type* src, size_t src_pitch, dpct::queue_ptr stream)
{
    const size_t dst_pitch      = sx * sizeof(Type);
    const size_t width_in_bytes = sx * sizeof(Type);
...
struct CudaTexture
{
    dpct::image_wrapper_base_p textureObj = 0;
...
struct CudaRGBATexture
{
    ...
    CudaRGBATexture(CudaDeviceMemoryPitched<CudaRGBA, 2>& buffer_dmp)
     try {
        ...
        memset(&texDesc, 0, sizeof(dpct::sampling_info));
...
#else
        /*
        IGNORED- DPCT1007:0: Migration of cudaTextureDesc::readMode is not supported.
        */
        // texDesc.readMode = cudaReadModeElementType;
#endif

#if defined ALICEVISION_DEPTHMAP_TEXTURE_USE_INTERPOLATION
        texDesc.set(sycl::filtering_mode::linear);
#else
        texDesc.filterMode = cudaFilterModePoint;
#endif

        dpct::image_data resDesc;
        resDesc.set_data_type(dpct::image_data_type::pitch);

#ifdef ALICEVISION_DEPTHMAP_TEXTURE_USE_HALF
        resDesc.set_channel(cudaCreateChannelDescHalf4());
#else
        resDesc.res.pitch2D.desc = cudaCreateChannelDesc<CudaRGBA>();
#endif

        resDesc.set_data_ptr(buffer_dmp.getBuffer());
        resDesc.set_x(buffer_dmp.getSize()[0]);
        resDesc.set_y(buffer_dmp.getSize()[1]);
        resDesc.set_pitch(buffer_dmp.getPitch());

        // create texture object
        // note: we only have to do this once
        CHECK_CUDA_RETURN_ERROR(DPCT_CHECK_ERROR(textureObj = dpct::create_image_wrapper(resDesc, texDesc)));
    }
    ...
    ~CudaRGBATexture()
     try {
        CHECK_CUDA_RETURN_ERROR_NOEXCEPT(DPCT_CHECK_ERROR(delete textureObj));
    }
    catch(sycl::exception const& exc) {
      std::cerr << exc.what() << "Exception caught at file:" << __FILE__ << ", line:" << __LINE__ << std::endl;
      std::exit(1);
