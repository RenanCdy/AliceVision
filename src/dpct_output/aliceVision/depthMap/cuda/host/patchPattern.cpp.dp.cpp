...
void buildCustomPatchPattern(const CustomPatchPatternParams& patchParams)
 try {
    ...
    cudaMallocHost(&patchPattern_h, sizeof(DevicePatchPattern));
    ...
    DPCT_CHECK_ERROR(dpct::get_default_queue()
                            .memcpy(constantPatchPattern_d.get_ptr(), patchPattern_h, sizeof(DevicePatchPattern))
                            .wait());

    // free host-side patch pattern struct
    DPCT_CHECK_ERROR(sycl::free(patchPattern_h, dpct::get_default_queue()));
}
