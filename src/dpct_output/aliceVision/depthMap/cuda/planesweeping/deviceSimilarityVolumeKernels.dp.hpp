...
inline void move3DPointByRcPixSize(sycl::float3& p, const DeviceCameraParams& rcDeviceCamParams, const float rcPixSize)
{
    sycl::float3 rpv = dpct_operator_overloading::operator-(p, rcDeviceCamParams.C);
    normalize(rpv);
    p = dpct_operator_overloading::operator+(p, dpct_operator_overloading::operator*(rpv, rcPixSize));
}

inline void volume_computePatch(Patch& patch, const DeviceCameraParams& rcDeviceCamParams,
                                const DeviceCameraParams& tcDeviceCamParams, const float fpPlaneDepth,
...
inline float depthPlaneToDepth(const DeviceCameraParams& deviceCamParams, const float fpPlaneDepth,
                               const sycl::float2& pix)
{
    const sycl::float3 planep = dpct_operator_overloading::operator+(
        deviceCamParams.C, dpct_operator_overloading::operator*(deviceCamParams.ZVect, fpPlaneDepth));
    ...
    return size(dpct_operator_overloading::operator-(deviceCamParams.C, p));
}

template <typename T>
void volume_init_kernel(T* inout_volume_d, int inout_volume_s, int inout_volume_p,
...
void volume_add_kernel(TSimRefine* inout_volume_d, int inout_volume_s, int inout_volume_p, 
                                  ...
                                  const unsigned int volDimY, const sycl::nd_item<3> &item_ct1)
{
    ...
    *outSimPtr =
        sycl::vec<float, 1>{
            (sycl::vec<sycl::half, 1>{(*outSimPtr)}.convert<float, sycl::rounding_mode::automatic>()[0] +
             sycl::vec<sycl::half, 1>{(*get3DBufferAt(in_volume_d, in_volume_s, in_volume_p, vx, vy, vz))}
...
void volume_updateUninitialized_kernel(TSim* inout_volume2nd_d, int inout_volume2nd_s, int inout_volume2nd_p, 
...
/*
DPCT1110:15: The total declared local variable size in device function volume_computeSimilarity_kernel exceeds 128 bytes
...
void volume_computeSimilarity_kernel(
    TSim* out_volume1st_d, int out_volume1st_s, int out_volume1st_p, TSim* out_volume2nd_d, int out_volume2nd_s,
    int out_volume2nd_p, const float* in_depths_d, const int in_depths_p, const int rcDeviceCameraParamsId,
    const int tcDeviceCameraParamsId, const dpct::image_accessor_ext<sycl::float4, 2> rcMipmapImage_tex,
    const dpct::image_accessor_ext<sycl::float4, 2> tcMipmapImage_tex, const unsigned int rcSgmLevelWidth,
    const unsigned int rcSgmLevelHeight, const unsigned int tcSgmLevelWidth, const unsigned int tcSgmLevelHeight,
    const float rcMipmapLevel, const int stepXY, const int wsh, const float invGammaC, const float invGammaP,
    const bool useConsistentScale, const bool useCustomPatchPattern, const Range depthRange, const ROI roi,
    const sycl::nd_item<3>& item_ct1, DeviceCameraParams* constantCameraParametersArray_d,
    DevicePatchPattern constantPatchPattern_d)
{
    ...
    {
        fsim = compNCCby3DptsYK_customPatchPattern<invertAndFilter>(
            rcDeviceCamParams, tcDeviceCamParams, rcMipmapImage_tex, tcMipmapImage_tex, rcSgmLevelWidth,
            rcSgmLevelHeight, tcSgmLevelWidth, tcSgmLevelHeight, rcMipmapLevel, invGammaC, invGammaP,
            useConsistentScale, patch, constantPatchPattern_d);
    }
    else
    {
...
void volume_refineSimilarity_kernel(
    TSimRefine* inout_volSim_d, int inout_volSim_s, int inout_volSim_p, const sycl::float2* in_sgmDepthPixSizeMap_d,
    const int in_sgmDepthPixSizeMap_p, const sycl::float3* in_sgmNormalMap_d, const int in_sgmNormalMap_p,
    ...
    const dpct::image_accessor_ext<sycl::float4, 2> tcMipmapImage_tex, const unsigned int rcRefineLevelWidth,
    ...
    DeviceCameraParams* constantCameraParametersArray_d, DevicePatchPattern constantPatchPattern_d)
{
    const unsigned int roiX = item_ct1.get_group(2) * item_ct1.get_local_range(2) + item_ct1.get_local_id(2);
    const unsigned int roiY = item_ct1.get_group(1) * item_ct1.get_local_range(1) + item_ct1.get_local_id(1);
    const unsigned int roiZ = item_ct1.get_group(0);
    ...
    const sycl::float2 in_sgmDepthPixSize = *get2DBufferAt(in_sgmDepthPixSizeMap_d, in_sgmDepthPixSizeMap_p, vx, vy);
    ...
    // computeRotCSEpip
    {
      ...
      sycl::float3 v1 = dpct_operator_overloading::operator-(rcDeviceCamParams.C, patch.p);
      // vector from the target camera to the 3d point
      sycl::float3 v2 = dpct_operator_overloading::operator-(tcDeviceCamParams.C, patch.p);
      normalize(v1);
      normalize(v2);
      ...
      {
        patch.n = dpct_operator_overloading::operator/((dpct_operator_overloading::operator+(v1, v2)), 2.0f);
        normalize(patch.n);
      }
    ...
    if(useCustomPatchPattern)
    {
        fsimInvertedFiltered = compNCCby3DptsYK_customPatchPattern<invertAndFilter>(
            rcDeviceCamParams, tcDeviceCamParams, rcMipmapImage_tex, tcMipmapImage_tex, rcRefineLevelWidth,
...
void volume_retrieveBestDepth_kernel(sycl::float2* out_sgmDepthThicknessMap_d, int out_sgmDepthThicknessMap_p,
                                     ...
                                     const int in_volSim_s, const int in_volSim_p, const int rcDeviceCameraParamsId,
                                     ...
                                     DeviceCameraParams* constantCameraParametersArray_d)
{
    const unsigned int vx = item_ct1.get_group(2) * item_ct1.get_local_range(2) + item_ct1.get_local_id(2);
    const unsigned int vy = item_ct1.get_group(1) * item_ct1.get_local_range(1) + item_ct1.get_local_id(1);
    ...
    sycl::float2* out_bestDepthThicknessPtr =
        get2DBufferAt(out_sgmDepthThicknessMap_d, out_sgmDepthThicknessMap_p, vx, vy);

    // corresponding output depth/sim pointer or nullptr
    sycl::float2* out_bestDepthSimPtr =
        (out_sgmDepthSimMap_d == nullptr) ? nullptr : get2DBufferAt(out_sgmDepthSimMap_d, out_sgmDepthSimMap_p, vx, vy);
...
void volume_refineBestDepth_kernel(
    sycl::float2* out_refineDepthSimMap_d, int out_refineDepthSimMap_p, const sycl::float2* in_sgmDepthPixSizeMap_d,
    int in_sgmDepthPixSizeMap_p, const TSimRefine* in_volSim_d, int in_volSim_s, int in_volSim_p, int volDimZ,
    ...
    float twoTimesSigmaPowerTwo, const ROI roi, const sycl::nd_item<3>& item_ct1)
{
    ...
    sycl::float2* out_bestDepthSimPtr = get2DBufferAt(out_refineDepthSimMap_d, out_refineDepthSimMap_p, vx, vy);
...
template <typename T>
void volume_initVolumeYSlice_kernel(T* volume_d, int volume_s, int volume_p, const sycl::int3 volDim,
...
void volume_getVolumeXZSlice_kernel(T1* slice_d, int slice_p, const T2* volume_d, int volume_s, int volume_p,
...
void volume_computeBestZInSlice_kernel(TSimAcc* xzSlice_d, int xzSlice_p, TSimAcc* ySliceBestInColCst_d, int volDimX, int volDimZ,
                                       const sycl::nd_item<3> &item_ct1)
{
...
void volume_agregateCostVolumeAtXinSlices_kernel(
    const dpct::image_accessor_ext<sycl::float4, 2> rcMipmapImage_tex, const unsigned int rcSgmLevelWidth,
    const unsigned int rcSgmLevelHeight, const float rcMipmapLevel, TSimAcc* xzSliceForY_d, int xzSliceForY_p,
    const TSimAcc* xzSliceForYm1_d, const int xzSliceForYm1_p, const TSimAcc* bestSimInYm1_d, TSim* volAgr_d,
    const int volAgr_s, const int volAgr_p, const sycl::int3 volDim, const sycl::int3 axisT, const float step,
    ...
    const sycl::nd_item<3>& item_ct1)
{
    ...
    if((z >= 1) && (z < volDim.z() - 1))
    {
        ...
        else
        {
          ...
          const sycl::float4 gcr0 = _tex2DLod<sycl::float4>(rcMipmapImage_tex, float(imX0), rcSgmLevelWidth,
                                                            float(imY0), rcSgmLevelHeight, rcMipmapLevel);
          const sycl::float4 gcr1 = _tex2DLod<sycl::float4>(rcMipmapImage_tex, float(imX1), rcSgmLevelWidth,
