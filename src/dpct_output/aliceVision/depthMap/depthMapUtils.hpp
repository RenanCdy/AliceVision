...
void writeNormalMapFiltered(int rc, const mvsUtils::MultiViewParams& mp, const mvsUtils::TileParams& tileParams,
                            const ROI& roi, const CudaDeviceMemoryPitched<sycl::float3, 2>& in_normalMap_dmp,
                            int scale = 1, int step = 1, const std::string& name = "");
 ...
 */
void resetDepthSimMap(CudaHostMemoryHeap<sycl::float2, 2>& inout_depthSimMap_hmh, float depth = -1.f, float sim = 1.f);
