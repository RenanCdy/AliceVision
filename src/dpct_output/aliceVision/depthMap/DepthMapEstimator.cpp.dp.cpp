...
int DepthMapEstimator::getNbSimultaneousTiles() const
{
  ...
  sycl::queue& q_ct1 = dev_ct1.default_queue();
...

    {
      ...
      Sgm sgm(_mp, _tileParams, _sgmParams, sgmComputeDepthSimMap, sgmComputeNormalMap, &q_ct1 /*stream*/);
    ...
    {
      Refine refine(_mp, _tileParams, _refineParams, &q_ct1 /*stream*/);
  ...
  dpct::device_ext& dev_ct1 = dpct::get_current_device();
    ...
    for(int b = 0; b < nbBatches; ++b)
    {
        ...
        for(int i = firstTileIndex; i < lastTileIndex; ++i)
        {
            ...
            CudaHostMemoryHeap<sycl::float2, 2>& tileDepthSimMap_hmh =
