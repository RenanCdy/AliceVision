...
public:
     ...
     */
    Refine(const mvsUtils::MultiViewParams& mp, const mvsUtils::TileParams& tileParams,
           const RefineParams& refineParams, dpct::queue_ptr stream);
    ...
    inline const CudaDeviceMemoryPitched<sycl::float2, 2>& getDeviceDepthSimMap() const {
        return _optimizedDepthSimMap_dmp;
    }
     ...
     */
    void refineRc(const Tile& tile, const CudaDeviceMemoryPitched<sycl::float2, 2>& in_sgmDepthThicknessMap_dmp,
                  const CudaDeviceMemoryPitched<sycl::float3, 2>& in_sgmNormalMap_dmp);
...
private:
     ...
     */
    void computeAndWriteNormalMap(const Tile& tile, const CudaDeviceMemoryPitched<sycl::float2, 2>& in_depthSimMap_dmp,
    ...
    CudaDeviceMemoryPitched<sycl::float2, 2> _sgmDepthPixSizeMap_dmp;   //< rc upscaled SGM depth/pixSize map
    CudaDeviceMemoryPitched<sycl::float2, 2> _refinedDepthSimMap_dmp;   //< rc refined and fused depth/sim map
    CudaDeviceMemoryPitched<sycl::float2, 2> _optimizedDepthSimMap_dmp; //< rc optimized depth/sim map
    CudaDeviceMemoryPitched<sycl::float3, 2>
        _sgmNormalMap_dmp; //< rc upscaled SGM normal map (for experimentation purposes)
    CudaDeviceMemoryPitched<sycl::float3, 2>
        _normalMap_dmp; //< rc normal map (for debug / intermediate results purposes)
    ...
    dpct::queue_ptr _stream;                                       //< stream for gpu execution
};
