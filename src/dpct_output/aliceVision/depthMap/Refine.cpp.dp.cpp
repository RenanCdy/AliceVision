...
Refine::Refine(const mvsUtils::MultiViewParams& mp, const mvsUtils::TileParams& tileParams,
...
void Refine::refineRc(const Tile& tile, const CudaDeviceMemoryPitched<sycl::float2, 2>& in_sgmDepthThicknessMap_dmp,
...
void Refine::exportVolumeInformation(const Tile& tile, const std::string& name) const
{
    ...
    CudaHostMemoryHeap<sycl::float2, 2> depthPixSizeMapSgmUpscale_hmh(_sgmDepthPixSizeMap_dmp.getSize());
