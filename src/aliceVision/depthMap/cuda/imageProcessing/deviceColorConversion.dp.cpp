...
void rgb2lab_kernel(sycl::accessor<sycl::uchar4, 2> inout_img_d,
                               unsigned int inout_img_p,
                               unsigned int width,
                               unsigned int height,
                               const sycl::nd_item<3> &item_ct1)
{
    ...
    // corresponding input CudaRGBA
    sycl::uchar4 rgb_uc = get2DBufferAt(inout_img_d, x, y);

    sycl::float3 rgb = make_float3(rgb_uc);
...
void rgb2lab_kernel(sycl::accessor<sycl::float4, 2> inout_img_d,
                               unsigned int inout_img_p,
                               unsigned int width,
                               unsigned int height,
                               const sycl::nd_item<3> &item_ct1)
{
    ...
    // corresponding input CudaRGBA
    sycl::float4 rgb_f = get2DBufferAt(inout_img_d, x, y);

    sycl::float3 rgb = make_float3(rgb_f);
