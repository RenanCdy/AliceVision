...
*/
sycl::float2 getCellSmoothStepEnergy(const __sycl::DeviceCameraParams& rcDeviceCamParams,
                                     //const dpct::image_accessor_ext<float, 2> in_depth_tex, 
                                     sycl::accessor<float, 2, sycl::access::mode::read> in_depth_tex,
                                     const sycl::float2& cell0,
                                     const sycl::float2& offsetRoi)
{
    ...
    const float d0 = get2DBufferAt(in_depth_tex, cell0.x(), cell0.y());
    ...
    const sycl::float2 cellL = cell0 +  sycl::float2(0.f, -1.f); // Left
    const sycl::float2 cellR = cell0 +  sycl::float2(0.f, 1.f);  // Right
    const sycl::float2 cellU = cell0 +  sycl::float2(-1.f, 0.f); // Up
    const sycl::float2 cellB = cell0 +  sycl::float2(1.f, 0.f);  // Bottom

    // get associated depths from depth texture
    // note: we do not use 0.5f offset because in_depth_tex use nearest neighbor interpolation
    const float dL = get2DBufferAt(in_depth_tex, cellL.x(), cellL.y());
    const float dR = get2DBufferAt(in_depth_tex, cellR.x(), cellR.y());
    const float dU = get2DBufferAt(in_depth_tex, cellU.x(), cellU.y());
    const float dB = get2DBufferAt(in_depth_tex, cellB.x(), cellB.y());

    // get associated 3D points
    const sycl::float3 p0 = get3DPointForPixelAndDepthFromRC(rcDeviceCamParams, cell0 + offsetRoi, d0);
    const sycl::float3 pL = get3DPointForPixelAndDepthFromRC(rcDeviceCamParams, cellL + offsetRoi, dL);
    const sycl::float3 pR = get3DPointForPixelAndDepthFromRC(rcDeviceCamParams, cellR + offsetRoi, dR);
    const sycl::float3 pU = get3DPointForPixelAndDepthFromRC(rcDeviceCamParams, cellU + offsetRoi, dU);
    const sycl::float3 pB = get3DPointForPixelAndDepthFromRC(rcDeviceCamParams, cellB + offsetRoi, dB);
    ...
    if(n > 1.0f)
    {
        ...
        sycl::float3 vcn = rcDeviceCamParams.C - p0;
        normalize(vcn);
...
void optimize_varLofLABtoW_kernel(sycl::accessor<float, 2, sycl::access::mode::write> out_varianceMap_d,
                                //float* out_varianceMap_d, int out_varianceMap_p,
                                  //const dpct::image_accessor_ext<sycl::float4, 2> rcMipmapImage_tex,
                                  sycl::accessor<sycl::float4, 2, sycl::access::mode::read, sycl::access::target::image> rcMipmapImage_tex,
                                  sycl::sampler sampler,
                                  ...
                                  const sycl::nd_item<3>& item_ct1)
{
    ...
    const float xM1 =
        _tex2DLod(rcMipmapImage_tex, sampler, (x - 1.f), rcLevelWidth, (y + 0.f), rcLevelHeight, rcMipmapLevel).x();
    const float xP1 =
        _tex2DLod(rcMipmapImage_tex, sampler, (x + 1.f), rcLevelWidth, (y + 0.f), rcLevelHeight, rcMipmapLevel).x();
    const float yM1 =
        _tex2DLod(rcMipmapImage_tex, sampler, (x + 0.f), rcLevelWidth, (y - 1.f), rcLevelHeight, rcMipmapLevel).x();
    const float yP1 =
        _tex2DLod(rcMipmapImage_tex, sampler, (x + 0.f), rcLevelWidth, (y + 1.f), rcLevelHeight, rcMipmapLevel).x();
    ...
    // write output
    get2DBufferAt(out_varianceMap_d, roiX, roiY) = grad;
}

void optimize_getOptDeptMapFromOptDepthSimMap_kernel(sycl::accessor<float, 2, sycl::access::mode::write> out_tmpOptDepthMap_d,
                                                    sycl::accessor<sycl::float2, 2, sycl::access::mode::read> in_optDepthSimMap_d,
                                                     ...
                                                     const sycl::nd_item<3>& item_ct1)
{
    ...
    get2DBufferAt(out_tmpOptDepthMap_d, roiX, roiY) =
        get2DBufferAt(in_optDepthSimMap_d, roiX, roiY).x(); // depth
}

void optimize_depthSimMap_kernel(sycl::accessor<sycl::float2, 2, sycl::access::mode::write> out_optimizeDepthSimMap_d,
                                sycl::accessor<sycl::float2, 2, sycl::access::mode::read> in_sgmDepthPixSizeMap_d,
                                sycl::accessor<sycl::float2, 2, sycl::access::mode::read> in_refineDepthSimMap_d,
                                //  sycl::float2* out_optimizeDepthSimMap_d,
                                ...
                                //  const int in_refineDepthSimMap_p, // input fine depth/sim map
                                 ...
                                 const sycl::accessor<float, 2, sycl::access::mode::read> imgVariance_tex,
                                 const sycl::accessor<float, 2, sycl::access::mode::read> depth_tex,
                                //  const dpct::image_accessor_ext<float, 2> imgVariance_tex,
                                //  const dpct::image_accessor_ext<float, 2> depth_tex, 
                                 const int iter, const ROI roi,
                                 const sycl::nd_item<3>& item_ct1, const __sycl::DeviceCameraParams* cameraParametersArray_d)
{
    ...
    const sycl::float2& sgmDepthPixSize = get2DBufferAt(in_sgmDepthPixSizeMap_d, roiX, roiY);
    const float sgmDepth = sgmDepthPixSize.x();
    const float sgmPixSize = sgmDepthPixSize.y();

    // refined and fused (fine) depth/sim
    const sycl::float2& refineDepthSim = get2DBufferAt(in_refineDepthSimMap_d, roiX, roiY);
    const float refineDepth = refineDepthSim.x();
    const float refineSim = refineDepthSim.y();

    // output optimized depth/sim
    sycl::float2& out_optDepthSimPtr = get2DBufferAt(out_optimizeDepthSimMap_d, roiX, roiY);
    ...
    if (depthOpt > 0.0f)
    {
        ...
        const float& imgColorVariance = get2DBufferAt(imgVariance_tex,
            float(roiX),
